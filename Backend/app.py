...
#         machine = session.query(Machine).filter_by(uuid=data['machine_id']).first()
#         system_info = data.get('system_info', {})
        
#         if not machine:
#             machine = Machine(
#                 uuid=data['machine_id'],
...
#             session.commit()
#         else:
#             machine.hostname = system_info.get('hostname', machine.hostname)
#             machine.os_platform = system_info.get('os_platform', machine.os_platform)
#             machine.os_version = system_info.get('os_version', machine.os_version)
#             machine.username = system_info.get('username', machine.username)
#             machine.cpu_cores = system_info.get('cpu_cores', machine.cpu_cores)
#             machine.memory_mb = system_info.get('memory_mb', machine.memory_mb)
#             machine.last_seen = data['timestamp']
        
#         report = Report(
#             machine_id=machine.id,
#             timestamp=data['timestamp'],
#             disk_encryption=data['state'].get('disk_encryption', False),
#             os_updates=data['state'].get('os_updates', False),
#             antivirus=data['state'].get('antivirus', False),
#             sleep_settings=data['state'].get('sleep_settings', False),
#             cpu_usage=data['metrics'].get('cpu_usage', 0),
#             memory_usage=data['metrics'].get('memory_usage', 0),
#             disk_usage=data['metrics'].get('disk_usage', 0),
#             disk_info=json.dumps(data['metrics'].get('disks', [])),
#             raw=data.get('raw', '')
#         )
#         session.add(report)
#         session.commit()
...
#                 disk_info = []
#                 try:
#                     disk_info = json.loads(last_report.disk_info) if last_report.disk_info else []
#                 except:
#                     pass
#                 result.append({
...
#             'cpu_cores', 'memory_mb', 'last_seen', 'disk_encryption', 'os_updates',
#             'antivirus', 'sleep_settings', 'cpu_usage', 'memory_usage', 'disk_usage'
#         ])
