...
def report_data():
    ...
    try:
        ...
        if not machine:
            ...
            # Reactivate machine automatically when it reports again
            machine.active = True
            session.add(machine)
            session.commit()
        ...
        if (not state or state == {}) and (not metrics or metrics == {}) and (not disk_info):
    
            return jsonify({"status": "heartbeat"}), 200
...
@app.route("/api/machines/<string:machine_id>/deactivate", methods=["POST"])
def deactivate_machine(machine_id):
    session = db_session()
    try:
        machine = session.query(Machine).filter_by(uuid=machine_id).first()
        if not machine:
...
@app.route("/api/machines/<string:machine_id>/reactivate", methods=["POST"])
def reactivate_machine(machine_id):
    session = db_session()
    try:
        machine = session.query(Machine).filter_by(uuid=machine_id).first()
        if not machine:
...
def get_machines():
    ...
    try:
        try:
            machines = session.query(Machine).filter_by(active=True).all()
        except Exception:
...
def command_result(cmd_id):
    ...
    orig_cmd = None
    if mid and mid in commands_store:
        for c in commands_store.get(mid, []):
            if c.get("id") == cmd_id:
                orig_cmd = c
                break
    ...
    if orig_cmd and orig_cmd.get("action") == "kill" and success and mid:
        session = db_session()
        try:
            machine = session.query(Machine).filter_by(uuid=mid).first()
            if machine:
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=5000, debug=False, ssl_context=(str(SERVER_CERT), str(SERVER_KEY)))
